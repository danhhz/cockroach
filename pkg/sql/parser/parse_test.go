...
func TestParse(t *testing.T) {
	testData := []struct {
		...
		{`CREATE TABLE a (b SERIAL) PARTITION BY LIST (b) (PARTITION c (1))`},
		{`CREATE TABLE a (b SERIAL) PARTITION BY RANGE (b) (PARTITION c VALUES LESS THAN (1), PARTITION d VALUES LESS THAN (2))`},
